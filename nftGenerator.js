...
const getNFT = async (gameId, pgn, outcome, w, b) => {
  try {
    ...
    framePieceCtx.drawImage(await piece_skin_bg, 0, 0);

    let piece_skin = await loadImage(frame2.toBuffer("image/png"));
    framePieceCtx.drawImage(await piece_skin, 0, 0);

    let piece_skin_fg = loadImage(`./inputs/${lmPiece}_fg.png`);
    framePieceCtx.drawImage(await piece_skin_fg, 0, 0);
    ...
    const nftData = {
      ...
      attributes: [
        ...
        },
        {
          ...
          value: `${lmove.color == "w" ? "White" : "Black"} ${
            pieceMap[lmPiece]
          }`,
        },
        {
      ...
      piece: new File(
        [pieceSkin],
        `${lmove.color}${lmPiece.toUpperCase()}.png`,
        {
          type: "image/png",
        }
      ),
